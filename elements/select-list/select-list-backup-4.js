...
class SelectListUtils {
  ...
  class SelectList extends HTMLElement {
    ...
    init() {
      ...
      if (this.optionsAttributeTypeEnum === SelectListUtils.optionsAttributeTypeEnum.innerHtml) {
        this.updateOptions('');
      } else {
        this.updateOptions(optionsAttribute);
      }
    }
    async updateOptions(newValue) {
      ...
      try {
        ...
        this.options = [];
        this.filteredOptions = [];
        this.renderErrorMessage('Error loading options');

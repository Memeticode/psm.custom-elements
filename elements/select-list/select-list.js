...
class SelectListUtils {
  ...
  static async getOptionsFromInnerHtml(selectListElement, search) {
    const optionElements = selectListElement.querySelectorAll('option');  
    const optionsData = Array.from(optionElements).map(option => ({
      value: option.value,
      label: option.textContent.trim()
    }));  
    return this.filterOptions(optionsData, search);
  }
  ...
  static async getOptionsFromJson(options, search) {
    ...
    if (Array.isArray(parsedOptions)) {
      if (typeof parsedOptions[0] === 'object') {
        ...
        return this.filterOptions(parsedOptions.map(value => ({ value, label: value })), search);
      }
    } else {
      throw new Error('Invalid JSON options format');
    }
  }
  ...
  static determineOptionsType(options) {
    try {
      if (!options || options === null || options === undefined || options.trim() === '') {
  ...
  static async getOptions(options, optionsTypeEnum, search, urlHttpHeaders, searchParamName, selectListElement) {
    switch (optionsTypeEnum) {
      case this.optionsAttributeTypeEnum.innerHtml:
        return this.getOptionsFromInnerHtml(selectListElement, search);
...
class SelectList extends HTMLElement {
  constructor() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .no-options, .loading-state, .error-state {
          padding: 8px;
          font-style: italic;
          color: #999;
          cursor: default;
    ...
    this.lastLoadedOptionsAttribute = null;
  ...
  async updateOptions(newValue) {
    ...
    try {
      ...
      this.options = [];
      this.filteredOptions = [];
      this.renderErrorState('Error loading options');
      this.logError('Error loading options', error);
      this.setOptionsStateError();
    }
  }
  ...
  renderLoadingState() {
    ...
    this.optionsList.appendChild(loadingElement);
  }
  ...
  async loadOptions() {
    ...
    try {
      ...
      this.lastLoadedOptionsAttribute = optionsAttribute;
    } catch (error) {
      this.options = [];
      this.filteredOptions = [];
      this.optionsStateEnum = SelectListUtils.optionsStateEnum.error;
      this.logError('Error loading options', error);
  ...
  async showOptions() {
    if (this.optionsStateEnum === SelectListUtils.optionsStateEnum.unloaded || this.hasOptionsAttributeChanged()) {
      await this.loadOptions();
    }
    ...
    if (this.optionsStateEnum === SelectListUtils.optionsStateEnum.loaded) {
      ...
      this.optionsMenuStateEnum = SelectListUtils.optionsMenuStateEnum.open;
    }
  }

  hasOptionsAttributeChanged() {
    const currentOptionsAttribute = this.getAttribute('options');
    return currentOptionsAttribute !== this.lastLoadedOptionsAttribute;
  }
  ...
  setOptionsStateLoading() {
    this.optionsStateEnum = SelectListUtils.optionsStateEnum.loading;
    this.renderOptions();
  }
  
  setOptionsStateError() {
    this.optionsStateEnum = SelectListUtils.optionsStateEnum.error;
    this.renderOptions();
  }
  ...
  async filterOptions() {
    ...
    try {
      ...
      this.filteredOptions = await SelectListUtils.getOptions(this.getAttribute('options'), this.optionsAttributeTypeEnum, searchTerm, this.urlHttpHeaders, this.searchParamName, this);
      this.highlightedIndex = 0;
      this.optionsStateEnum = SelectListUtils.optionsStateEnum.loaded; // Set options state to loaded
      this.renderOptions();
    } catch (error) {
